...
void SmoothMotionCompute(uint ids : SV_DispatchThreadID)
{
    ...
    float time = ConstantUnsortedTypes[0];
    float RatioRMS = ConstantUnsortedTypes[1];
    float RealSound = ConstantUnsortedTypes[2];
    ...
    if (id*rem< 10*RatioRMS*1024) {

        posModx -= RealSound/100;
        posMody -= RealSound/100;
        posModz -= RealSound/100;
